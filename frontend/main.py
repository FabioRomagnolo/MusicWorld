...
def index():
    ...
    title = 'Music World | Songs, lyrics, news & more'
    form = SearchForm()
...
def artist(artist_id):
    ...
    if genius is not None:
        ...
        annotations = genius.get('annotations', None)
        if annotations:
            # Registered users will see full artist annotations, instead of just a link.
            alternate_names = annotations.get('alternate_names', None)
            description = format_genius_html(annotations.get('description', None))

            context.update({
                'alternate_names': alternate_names, 'description': description,
            })
    ...
    context.update({
        'albums': albums, 'top_tracks': top_tracks, 'news': news, 'links': links
    })
...
def album(album_id):
    ...
    if genius:
        ...
        annotations = genius.get('annotations', None)
        if annotations:
            # Registered users will see full album annotations, instead of just a link.
            description = format_genius_html(annotations.get('description', None))
            producers = annotations.get('producers', None)
            writers = annotations.get('writers', None)
            labels = annotations.get('labels')

            context.update({
                'description': description, 'producers': producers, 'writers': writers, 'labels': labels
            })
...
def track(track_id):
    ...
    if request.method == 'GET':
        ...
        if genius:
            ...
            annotations = genius.get('annotations', None)
            if annotations:
                # Registered users will see full YouTube video, annotations, and lyrics instead of just links.
                # lyrics = genius.get('lyrics', None)

                description = format_genius_html(annotations.get('description', None))
                producers = annotations.get('producers', None)
                writers = annotations.get('writers', None)
                labels = annotations.get('labels', None)
        ...
        context.update({
            'video': video, 'news': news, 'links': links
        })
